...
export function sendCode(data: { target: string }) {
  return request.post<unknown, { message: string; code: string }>('/auth/send-code', data)
}

export function sendLoginCode(data: { target: string }) {
  return request.post<unknown, { message: string; code: string }>('/auth/send-login-code', data)
}
